import { createSideBorderParser } from '../src/compute/dsl/compileParser'
import { testEngine, Case, failure, success } from './lib/dslTestEngine'
import {
   SideBorder,
   BorderState,
   BorderRootGroup,
} from '../src/compute/dsl/BorderType'
...
let zero: BorderState = { type: 'state', ...cMap([1, 1]), ...qw1 }
let one: BorderState = { type: 'state', ...cMap([0, 1]), ...qw1 }
let rand01: BorderState = { type: 'state', ...cMap([1, 2]), ...qw1 }
...
let borderPatternList: Case<string, SideBorder>[] = [
   success('(0)', {
      init: emptyGroup,
   ...
   success('(0[0](0){1})', {
      ...
      cycle: {
         ...
         content: [
            zero,
            zero,
            { type: 'group', content: [zero], quantity: 1, width: 1 },
         ],
         quantity: 1,
         width: 3,
      },
   ...
   success('(01{31})', {
      ...
      cycle: {
         ...
         content: [
            zero,
            { type: 'state', ...cMap([0, 1]), quantity: 31, width: 31 },
         ],
         quantity: 1,
         width: 32,
      },
   ...
   success('1{3}(011{4})', {
      ...
      cycle: {
         ...
         content: [
            zero,
            one,
            { type: 'state', ...cMap([0, 1]), quantity: 4, width: 4 },
         ],
         quantity: 1,
         width: 6,
      },
   ...
   success('1{3}(11(01){4})', {
      ...
      cycle: {
         ...
         content: [
            one,
            one,
            { type: 'group', content: [zero, one], quantity: 4, width: 8 },
         ],
         quantity: 1,
         width: 10,
      },
